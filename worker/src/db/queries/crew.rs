...
pub async fn create_crew(
    ...
    let crew = sqlx::query_as::<_, Crew>(
        r#"
        INSERT INTO crews (user_id, name, home_depot_id, preferred_areas, working_hours_start, working_hours_end, arrival_buffer_percent)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING id, user_id, name, home_depot_id, preferred_areas, 
                  working_hours_start, working_hours_end, is_active, arrival_buffer_percent, created_at, updated_at
        "#
    )
    .bind(user_id)
    .bind(&request.name)
