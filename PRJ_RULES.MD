...
- **i18n**: Translation key parity across locales, formatter output, resolveBackendMessage handling
...
│       │   ├── i18n/            # i18n instance, formatters, resolveBackendMessage (see PRJ_I18N.MD)
│       │   ├── lib/            # Utilities
...
> Full architecture documented in `PRJ_I18N.MD`.

### Translation Keys
- **Convention:** `namespace:section.action_or_noun` (colon separates namespace, dots separate hierarchy)
- **Namespaces:** `common`, `auth`, `nav`, `calendar`, `customers`, `planner`, `settings`, `import`, `jobs`, `validation`
- **English is the default/fallback language** — all `en/*.json` files are the source of truth
- Keys use `snake_case`: `planner:filters.due_this_week`

### Translation Files
- Location: `apps/web/public/locales/{lng}/{namespace}.json`
- All locales must have identical key sets — enforced by `scripts/check-i18n-keys.ts`
- Never commit a key to one locale without adding it to all others

### Component Rules
- Use `useTranslation(namespace)` hook in React components
- Use `i18n.t()` directly in stores, services, and non-React code
- Never hardcode user-visible strings — always use `t()` or formatter functions
- Date/time/number formatting: always use `i18n/formatters.ts`, never `toLocaleDateString('cs-CZ')` or similar

### Backend Messages
- Rust backend sends `{ key: "namespace:key.path", params: { ... } }` — never localized strings
- Frontend resolves via `resolveBackendMessage()` helper
- Legacy plain strings (pre-migration) are passed through as-is

### Shared Types
- Label maps use key maps (`*_KEYS`) pointing to translation keys, not `*_LABELS` with hardcoded Czech
- Example: `DEVICE_TYPE_KEYS: Record<DeviceType, string>` where values are `'common:device_type.boiler'` etc.
...
### i18n
- [ ] No new hardcoded user-visible strings — use `t()` keys
- [ ] New keys added to both `en` and `cs` locale files
- [ ] Date/time/number formatting uses `formatters.ts`, not inline `toLocaleString`
- [ ] Backend messages use `{ key, params }` shape, not localized strings
- [ ] `scripts/check-i18n-keys.ts` passes (key parity)
...
### PanelState Sync (Plan ↔ Map)

When a page (Inbox, Plan) manages local state that the map panel needs, every
relevant piece of data must be bridged into `PanelStateContext` via the
corresponding `actions.*` call. The map panel wrapper (`panels/RouteMapPanel`)
reads exclusively from context — it has no props from the host page.

Checklist for adding a new page or route that uses the map panel:
- [ ] `actions.setRouteStops(stops)` — route stops
- [ ] `actions.setRouteGeometry(geo)` — decoded polyline
- [ ] `actions.setMapDepot(depot)` — depot coordinates and name
- [ ] `actions.selectCustomer(id)` — stop/customer highlight (bidirectional with map)
- [ ] `actions.highlightSegment(idx)` — segment highlight (bidirectional with map)
- [ ] `actions.setRouteContext(ctx)` — date, crew, depot IDs

---

## Bug-Bashing Process

When investigating and fixing bugs, follow this disciplined process:

### 1. Document
- Record the bug in `PRJ_BUGS.MD` with: page, steps to reproduce, expected vs
  actual behavior, and screenshots if available.
- Do not start fixing until the bug is clearly described.

### 2. Red — write a failing test
- Write one or more tests that **detect** the bug (the test must fail before
  the fix and pass after).
- Prefer the narrowest scope that still catches the bug:
  - **Pure utility** → unit test in the same `*.test.ts` file.
  - **State sync / prop forwarding** → integration test that renders the page
    or panel wrapper and asserts on captured `PanelState` or mock-component
    props.
  - **Visual / MapLibre** → component test with a mocked MapLibre `Map` that
    captures `addSource`/`addLayer` calls.

### 3. Green — fix the bug
- Write the minimum code change that makes all tests pass.
- Run the full affected test suite (`vitest run <file>`) to confirm no
  regressions.

### 4. Manual verification
- Deploy (commit and push) so the fix can be tested in production.
- Let the reporter verify the fix before closing.

### 5. Guard
- After manual confirmation, evaluate whether additional edge-case or
  integration tests are needed to prevent the bug from recurring.
- Update `PRJ_BUGS.MD` with: root cause, fix description, list of regression
  test names and files.
- All tests created during bug-bashing are **permanent regression guards**, not
  throwaway instrumentation.

### Anti-patterns
- **No extraction for testability alone.** Do not extract trivial inline code
  (e.g., a `.map()` callback) into a named utility just to unit-test it.
  Prefer testing at the component/integration level.
- **No debug instrumentation in commits.** Console logs, fetch wrappers, or
  `_log` helpers used during investigation must be removed before the final
  commit. If production logging is needed, use the project's `logger` utility.
...
1. **Minimal base images** — use `-slim`, `-alpine`, or `distroless`
2. **Multi-stage builds** — separate build tools from runtime
3. **Derive version from project** — use git SHA or Cargo.toml version, never hardcode
4. **Cache Docker layers** — base image first, dependencies before source
5. **Combine & chain RUN commands** — clean caches in the same layer (`rm -rf /var/lib/apt/lists/*`)
6. **Explicit COPY of specific files** — never `COPY . .` without `.dockerignore`
7. **Production dependencies only** — no dev deps, no test frameworks in runtime
8. **Non-root user** — always `USER <name>` before `CMD`
9. **Pin image versions** — never `:latest`, use explicit tags or SHA digests
10. **Official images only** — use Docker Hub official or verified publisher images
11. **No secrets in image** — use env vars or mounted secrets, never bake in
12. **No sudo** — never install or use sudo in containers
13. **Minimal packages** — only what's strictly needed for runtime
14. **COPY over ADD** — ADD has implicit tar extraction and URL fetching; use COPY
15. **No debugging tools** — no `curl`, `vim`, `strace` in production images
16. **Executables owned by root, run by non-root** — `COPY --chown=root:root`, then `USER`
17. **Sort arguments alphabetically** — in `apt-get install`, `apk add`, etc.
18. **Use WORKDIR** — never `RUN cd /path && ...`
19. **Exec form for CMD** — `CMD ["binary"]`, not `CMD binary` (shell form)
20. **Comment non-obvious decisions** — explain why, not what
21. **Add OCI labels** — `org.opencontainers.image.*` for metadata
