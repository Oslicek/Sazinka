...
**Role:** `admin` (systém), `customer` (majitel firmy), `worker` (zaměstnanec).
Worker má `owner_id` odkazující na svého customera – vidí stejná data.
...
├── is_anonymized     BOOLEAN NOT NULL DEFAULT FALSE       ← nové (soft delete)
├── anonymized_at     TIMESTAMPTZ                          ← nové (audit stopa)
├── notes             TEXT
├── created_at        TIMESTAMPTZ
└── updated_at        TIMESTAMPTZ
```
...
**Poznámka k mazání zákazníka (aktuální implementace):**

- `customers` se při "smazání" fyzicky nemažou.
- Záznam je anonymizován (`is_anonymized = TRUE`, `anonymized_at = NOW()`), PII pole jsou vyčištěna.
- Čtecí dotazy pro zákazníky filtrují `is_anonymized = FALSE`, takže anonymizovaní zákazníci nejsou vidět v UI.
- Vazby v dalších tabulkách zůstávají zachované (FK integrita), navazující PII je anonymizována.
- API kontrakt: `sazinka.customer.delete` vrací `deleted: true` i když jde o soft anonymizaci, nikoliv hard delete.
...
**Kritický problém – `device_name`:** Import specifikace (PROJECT_IMPORT.MD, sekce 11.6 a 15.3) definuje hierarchii identifikace zařízení: `serial_number` → `device_name` → `device_type` (pokud jen 1 zařízení typu). Navíc `device_name` je povinný, pokud má zákazník více zařízení, i když je vyplněn `serial_number`. Ale:
...
Import specifikace (PROJECT_IMPORT.MD) definuje `device_name` jako lidsky čitelný identifikátor zařízení. Databáze tento sloupec nemá. `resolve_device_ref()` hledá pouze podle `serial_number`, takže reference typu `"Komín na chalupě"` nebo `"Ohřívač koupelna"` v CSV se nikdy nenajdou.
