...
| `NATS_BROWSER_PASSWORD` | Stejná hodnota jako `NATS_BROWSER_PASSWORD` v `.env.prod` na VPS | Vite zapéká do SPA bundlu jako `VITE_NATS_PASS` |
...
- [ ] Root `/` vrací 302 redirect (edge function `functions/index.ts`, žádný flash)
...
### 24.15 Stav implementace CI/CD (aktualizováno 2026-03-08)

**Workflow soubory**:
- `.github/workflows/ci.yml` — CI na push (testy, audit, GHCR build). Implementováno a testováno.
- `.github/workflows/deploy.yml` — manuální deploy přes `workflow_dispatch` s Tailscale SSH. Implementováno.
- `.github/workflows/site.yml` — CI pro marketing site (testy, Lighthouse). Auto-deploy odstraněn.

**GitHub Secrets — stav nastavení**:
...
| `MIGRATION_DB_URL` | ✅ Nastaveno (`postgres://sazinka_admin:...@postgres:5432/sazinka`) |
| `MIGRATION_DB_URL_STAGING` | ⏳ Čeká na staging setup |
| `CF_API_TOKEN` | ✅ Nastaveno (custom token s Pages:Edit) |
| `CF_ACCOUNT_ID` | ✅ Nastaveno |

**Tailscale OAuth client**:
- Scopes: `devices:core`, `auth_keys` (read & write) — obojí je nutné.
- Tags: `tag:ci`.
- Pozor: scope `auth_keys:read` nestačí — GitHub Action potřebuje **write** pro vytvoření ephemeral auth key.

**Známé problémy vyřešené při prvním spuštění CI**:

1. **SQLx compile-time checks** — `sqlx::query!` makra ověřují dotazy proti DB při kompilaci.
   CI Postgres service nemá tabulky → nutné spustit migrace před Clippy/testy.
   Řešení: přidán krok `cargo sqlx migrate run` do `ci.yml` před Clippy.

2. **Clippy pre-existing warnings** — desítky existujících varování (`too_many_arguments`,
   `dead_code`, `items_after_test_module`, `manual_div_ceil`, `type_complexity` aj.).
   Řešení: `-D warnings` zachováno (nové varování = fail), ale přidány `-A` pro
   existující kategorie. Postupně zpřísňovat při refaktoringu.
   Kompletní allow list viz `.github/workflows/ci.yml`.

3. **CI path filter** — `ci.yml` se spouští jen při změnách v `worker/**`, `packages/**`,
   `apps/web/**`, `infra/**`. Změny v `.github/workflows/**` CI nespustí — pro retrigger
   je nutné změnit soubor ve sledovaném adresáři.

4. **cargo audit — `bytes` a `time` CVE** — `bytes` 1.11.0 (integer overflow) a `time` 0.3.46
   (DoS via stack exhaustion). Řešení: `cargo update -p bytes -p time`.
   `rsa` RUSTSEC-2023-0071 (Marvin Attack) nemá fix — tranzitivní dep přes `sqlx-mysql`
   (nepoužíváme). Ignorováno v `worker/.cargo/audit.toml`.

5. **pnpm/action-setup verze konflikt** — `version: latest` v workflow koliduje s
   `packageManager` v `package.json`. Řešení: odstranit `version` parametr z `pnpm/action-setup`,
   nechat action číst verzi z `package.json`.

6. **Hero test — zastaralý assertion** — test očekával "Rozmotat", ale headline byl změněn
   na "Rozmotejte". Řešení: aktualizovat test.

7. **Lighthouse CI** — `serve` nebyl nainstalován na runneru; cesta k `dist/` byla relativní
   k `apps/site/`, ne k repo root. Řešení: `npm install -g serve` + cesta `apps/site/dist`.
   Lighthouse assertions (color-contrast, errors-in-console) zatím neblokují pipeline
   (`continue-on-error: true`). Plán oprav viz `PRJ_ROADMAP.MD` Phase 10.5.

8. **Gitleaks false positives** — test token v `onboarding.rs` a `curl -u` příklad v
   `PRJ_DEVOPS.MD`. Řešení: `.gitleaks.toml` s path allowlistem.

9. **Trivy — zlib1g CVE-2023-45853** — CRITICAL CVE v Debian 12 base image, status
   `will_not_fix` (affects minizip, not used). Řešení: `.trivyignore` soubor.

**Konfigurační soubory přidané pro CI**:
- `.gitleaks.toml` — allowlist pro false positive secret detekce
- `.trivyignore` — ignorované CVE bez dostupného fixu
- `worker/.cargo/audit.toml` — ignorované Rust advisory bez fixu

### 24.16 Security hardening — priority a důležitost
...
### 24.17 Problémy vyřešené při prvním deployi (2026-03-09)

**10. Worker image not found v GHCR při deployi**

Problém: Deploy workflow se pokoušel stáhnout `ghcr.io/oslicek/sazinka-worker:<SHA>`,
ale image pro daný commit neexistoval v GHCR, protože CI `build-worker` job se spouští
jen při změnách v `worker/**` nebo `packages/**`.

Řešení (dvě části):
- **CI (`ci.yml`)**: `build-push-action` nyní pushuje image se dvěma tagy:
  `${{ github.sha }}` a `latest`. Tag `latest` vždy ukazuje na poslední CI-validovaný build.
- **Deploy (`deploy.yml`)**: Job `prepare` nyní obsahuje krok `Resolve worker image tag`,
  který přes `docker manifest inspect` ověří, zda existuje image pro aktuální SHA.
  Pokud ne, fallback na `latest`. Pokud ani `latest` neexistuje, worker deploy se přeskočí
  s varováním. Jobs `migrate-db` a `deploy-worker` mají podmínku
  `needs.prepare.outputs.worker_available == 'true'`.

**11. Wrangler-action selhání v pnpm workspace**

Problém: `cloudflare/wrangler-action@v3` interně spouští `pnpm add wrangler@3.90.0`,
což v pnpm workspace root selže s `ERR_PNPM_ADDING_TO_ROOT` (vyžaduje flag `-w`).
Nastavení `packageManager: npm` v akci nepomohlo — npm vrátil
`Cannot read properties of null (reading 'matches')` kvůli pnpm-managed `node_modules`.

Řešení: Nahrazení `cloudflare/wrangler-action@v3` přímým voláním `npx wrangler`.
Wrangler automaticky čte credentials z env vars `CLOUDFLARE_API_TOKEN`
a `CLOUDFLARE_ACCOUNT_ID`.

```yaml
- name: Deploy to Cloudflare Pages
  run: npx wrangler pages deploy apps/site/dist --project-name=ariadline-site --commit-dirty=true
  env:
    CLOUDFLARE_API_TOKEN: ${{ secrets.CF_API_TOKEN }}
    CLOUDFLARE_ACCOUNT_ID: ${{ secrets.CF_ACCOUNT_ID }}
```

**12. DB migrace — password authentication failed**

Problém: `MIGRATION_DB_URL` obsahoval správné heslo, ale migrace selhala s
`password authentication failed for user "sazinka_admin"`. Heslo bylo ověřeno přes
`docker exec sazinka-postgres psql "postgres://sazinka_admin:PWD@localhost:5432/sazinka"`,
ale tento test neověřuje heslo — výchozí `pg_hba.conf` Postgres Docker image používá
`trust` auth pro `localhost` (heslo se nekontroluje).

Řešení: Test hesla je nutné provádět z **jiného kontejneru** na Docker síti:
```bash
docker run --rm --network infra_sazinka_net postgres:16-alpine \
  psql "postgres://sazinka_admin:HESLO@postgres:5432/sazinka" -c "SELECT 1;"
```
Po resetu hesla přes `ALTER ROLE` a ověření z externího kontejneru migrace proběhla úspěšně.

> **Poučení**: Nikdy netestovat DB hesla zevnitř Postgres kontejneru — `trust` auth na
> localhost obejde jakékoliv heslo. Vždy testovat přes síť.
